...
const InfrastructureGridSection = () => {
  ...
  // Duplicate list to guarantee smooth seamless infinite ticker loop
  const duplicatedTickerImages =
    tickerImages.length > 0
      ? [...tickerImages, ...tickerImages, ...tickerImages, ...tickerImages]
      : [];
    ...
    <section id="facilities-section" className="py-6 md:py-10 pb-16 md:pb-24 bg-background text-foreground overflow-hidden">
      <div className="w-full">
        {loading ? (
          ...
          /* Smooth Slow Auto-Animated Ticker (140% Larger Size) */
          <div className="relative w-full overflow-x-auto scrollbar-none">
            <div className="flex w-max animate-scroll hover:[animation-play-state:paused] ease-linear">
              {duplicatedTickerImages.map((image, index) => (
                <div
                  key={`ticker-${image.id || index}-${index}`}
                  className="w-[420px] sm:w-[580px] md:w-[720px] lg:w-[820px] h-[280px] sm:h-[380px] md:h-[460px] lg:h-[500px] flex-shrink-0 px-3 md:px-4"
                >
                  <div
                    onClick={() => setSelectedImage(image)}
                    className="relative w-full h-full group overflow-hidden rounded-2xl md:rounded-3xl bg-white shadow-xl hover:shadow-2xl border border-slate-100/80 transition-all duration-300 cursor-pointer"
                  >
                    {/* 140% High-Definition Image */}
                    <img
                      src={image.src}
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-between p-6 md:p-8">
                      <div>
                        <span className="inline-block px-3 py-1 rounded-full bg-primary/90 text-white text-xs font-bold uppercase tracking-wider mb-2">
                          {image.category === 'ticker' ? 'Infrastructure Ticker' : 'Campus Infrastructure'}
                        </span>
