...
const InfrastructureGridSection = () => {
  ...
  // Autoplay plugin for continuous auto-scrolling
      ...
      <div className="w-full px-4 md:px-8 lg:px-[80px]">
        <div className="max-w-7xl mx-auto">
          {loading ? (
            ...
            /* Infinite Looping Auto-Scroll & Swipeable Carousel Ticker */
            <div className="relative w-full">
              ...
              >
                <CarouselContent className="-ml-4 md:-ml-6">
                  {tickerImages.map((image, index) => (
                    <CarouselItem
                      ...
                      className="pl-4 md:pl-6 basis-[92%] sm:basis-[82%] md:basis-[68%] lg:basis-[58%]"
                    >
                      <div
                        onClick={() => setSelectedImage(image)}
                        className="relative w-full h-[300px] sm:h-[380px] md:h-[460px] lg:h-[520px] group overflow-hidden rounded-2xl md:rounded-3xl bg-slate-900/5 backdrop-blur-xs border border-slate-200/80 shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer flex items-center justify-center p-2 md:p-3"
                      >
                        {/* Full Width Uncropped HD Image */}
                        <img
                          src={image.src}
                          alt={image.alt || image.title || 'Eye-Net Infrastructure Ticker'}
                          className="w-full h-full object-contain object-center rounded-xl md:rounded-2xl transition-transform duration-500 group-hover:scale-[1.02]"
                          loading="lazy"
                        />
                ...
                {/* Left / Right Infinite Carousel Navigation */}
                <CarouselPrevious className="absolute left-2 md:-left-5 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-slate-800 border border-slate-200 shadow-xl w-12 h-12 z-20" />
                <CarouselNext className="absolute right-2 md:-right-5 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-slate-800 border border-slate-200 shadow-xl w-12 h-12 z-20" />
              </Carousel>
            </div>
          ) : (
